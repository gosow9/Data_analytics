...
def ex_4(data):
    print(f"#########---Excercise 4.a---################")
    mean_data = np.mean(data)
    std_data = np.std(data)
    print(f"Mean: {mean_data:.4f} and std: {std_data:.4f} ")
    print(f"#########---Excercise 4.b---################")

    err_mean = mean_data/np.sqrt(len(data))
    err_std = std_data/(np.sqrt(2*len(data)-2))
    print(f"Uncertainty of mean: {err_mean:.4f} and std: {err_std:.4f} ")

    print(f"#########---Excercise 4.c---################")
    org_mean = 10
    org_std = 3
    mc_res = np.asarray([np.random.normal(org_mean, org_std, 100)
                        for i in range(1000)])
    mc_mean = np.mean(mc_res, axis=1)-org_mean
    mc_std = np.std(mc_res, axis=1)-org_std
    mc_mean_rms = np.sqrt(np.mean(mc_mean**2))
    mc_std_rms = np.sqrt(np.mean(mc_std**2))
    print(f"Uncertainty for the mean via rms: {mc_mean_rms:.4f}")
    print(f"Uncertainty for the std via rms: {mc_std_rms:.4f}")
    boot_data = []
    for i in range(1000):
        boot_data.append([data[np.random.randint(0, len(data))]
                         for i in range(len(data))])

    boot_mean = np.mean(boot_data, axis=1)
    boot_std = np.std(boot_data, axis=1)
    boot_mean_var = np.var(boot_mean)
    boot_std_var = np.var(boot_std)
    print(f"#########---Excercise 4.d---################")
    print(f"Uncertainty for the mean via bootstrap: {boot_mean_var:.4f}")
    print(f"Uncertainty for the std via bootstrap: {boot_std_var:.4f}")

    print(f"#########---Excercise 4.e---################")
    print("The monte carlo is probably the best one since we take different distributions in account")
